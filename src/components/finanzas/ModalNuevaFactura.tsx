...
export default function ModalNuevaFactura({ isOpen, onClose, onSave, pacienteId, pacienteNombre }: ModalNuevaFacturaProps) {
  const { usuarioActual } = useAuth();
  const [nit, setNit] = useState('CF');
  const [nombreFactura, setNombreFactura] = useState(pacienteNombre);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!concepto || !montoTotal || !usuarioActual) return;
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Nombre Factura</label>
              <input
                type="text"
                required
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none transition-all duration-300"

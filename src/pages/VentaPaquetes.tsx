...
import { ShoppingBag, Package, User, Search, CheckCircle } from 'lucide-react';
import type { PaqueteSesion, Paciente } from '../types';
import Toast from '../components/common/Toast';

export default function VentaPaquetes() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ isVisible: true, message: 'Paquete vendido y factura generada exitosamente.', type: 'success' });
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 tracking-tight flex items-center">
            <ShoppingBag className="mr-3 text-violet-600" size={32} />
            Venta de Paquetes
          </h2>
          <p className="text-slate-500 mt-1 ml-11">Asigna paquetes de sesiones a pacientes y genera cobros automáticamente.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            >
              ...
              {isSubmitting ? 'Procesando Venta...' : 'Confirmar Venta de Paquete'}
            </button>
          </div>
        </form>
      </div>
